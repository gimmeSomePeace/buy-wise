    ...
    inner class List {
        ...
        inner class BasicPagination {
            ...
            fun `should return products for owner`() = runTest {
                val ownerId = createOwner()
                val product1 = persistence.persist(product(ownerId = ownerId, name = "Product 1"))
                val product2 = persistence.persist(product(ownerId = ownerId, name = "Product 2"))
            ...
            fun `should return page with cursor when more items exist`() = runTest {
                ...
                persistence.persist(product(ownerId = ownerId, name = "Product 1"))
                persistence.persist(product(ownerId = ownerId, name = "Product 2"))
                persistence.persist(product(ownerId = ownerId, name = "Product 3"))
        ...
        inner class CursorPagination {
            ...
            fun `should paginate using cursor`() = runTest {
                ...
                val products = (1..5).map { i ->
                    persistence.persist(product(ownerId = ownerId, name = "Product $i"))
                }
        ...
        inner class Filters {
            ...
            fun `should filter by name contains`() = runTest {
                val ownerId = createOwner()
                persistence.persist(product(ownerId = ownerId, name = "Coffee Beans"))
                persistence.persist(product(ownerId = ownerId, name = "Tea Leaves"))
                persistence.persist(product(ownerId = ownerId, name = "Coffee Grounds"))

                val result = query.list(
                    request = PageRequest(pageSize = 20),
                    filters = ProductFilters(nameContains = "Coffee", ownerId = ownerId),
                )

                assertThat(result.items).hasSize(2)
                assertThat(result.items.map { it.name })
                    .containsExactlyInAnyOrder("Coffee Beans", "Coffee Grounds")
            }
            ...
            fun `should filter by user id`() = runTest {
                ...
                persistence.persist(product(ownerId = userId1, name = "Coffee Beans"))
                persistence.persist(product(ownerId = userId1, name = "Tea Leaves"))
                persistence.persist(product(ownerId = userId2, name = "Coffee Grounds"))
            ...
            fun `should combine filters`() = runTest {
                ...
                persistence.persist(product(ownerId = userId1, name = "Tea Leaves"))
                persistence.persist(product(ownerId = userId2, name = "Coffee Grounds"))
                persistence.persist(product(ownerId = userId2, name = "Tea Leaves"))
            ...
            fun `should apply filters with cursor pagination`() = runTest {
                ...
                val coffeeProducts = listOf(
                    persistence.persist(product(ownerId = ownerId, name = "Coffee 1")),
                    persistence.persist(product(ownerId = ownerId, name = "Coffee 2")),
                    persistence.persist(product(ownerId = ownerId, name = "Coffee 3")),
                )
                persistence.persist(product(ownerId = ownerId, name = "Tea Leaves"))
