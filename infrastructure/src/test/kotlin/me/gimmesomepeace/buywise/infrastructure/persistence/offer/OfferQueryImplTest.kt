...
internal class OfferQueryImplTest : PostgresSqlContainer() {
    ...
    private fun createStoreWithOwner(ownerId: UserId): StoreId {
        return persistence.persist(store(ownerId = ownerId)).id
    }

    private fun createProductWithOwner(ownerId: UserId): ProductId {
        return persistence.persist(product(ownerId = ownerId)).id
    }
    ...
    inner class List {
        ...
        inner class BasicPagination {
            ...
            @Test
            fun `should not return offers of other users`() = runTest {
                val ownerId = createOwner()
        ...
        inner class OwnerFilter {
            ...
            @Test
            fun `should filter by owner id`() = runTest {
            ...
            @Test
            fun `should return empty when owner has no offers`() = runTest {
                val ownerId = createOwner()
                val anotherOwner = createOwner()
                createOfferWithOwner(anotherOwner)
        ...
        inner class ProductIdsFilter {
            ...
            @Test
            fun `should filter by single product id`() = runTest {
                val ownerId = createOwner()
                val productId = createProductWithOwner(ownerId)
                val storeId = createStoreWithOwner(ownerId)

                val offer1 = persistence.persist(
            ...
            @Test
            fun `should filter by multiple product ids`() = runTest {
                val ownerId = createOwner()
                ...
                val offer2 = persistence.persist(
            ...
            @Test
            fun `should return empty when product ids not match`() = runTest {
                val ownerId = createOwner()
                createOfferWithOwner(ownerId)
            ...
            @Test
            fun `should not return offers of other users even with matching product ids`() = runTest {
                val ownerId = createOwner()
                val anotherOwner = createOwner()
        ...
        inner class StoreIdsFilter {
            ...
            @Test
            fun `should filter by single store id`() = runTest {
                val ownerId = createOwner()
                val storeId = createStoreWithOwner(ownerId)
                val productId = createProductWithOwner(ownerId)
            ...
            @Test
            fun `should filter by multiple store ids`() = runTest {
                val ownerId = createOwner()
            ...
            @Test
            fun `should return empty when store ids not match`() = runTest {
                val ownerId = createOwner()
                createOfferWithOwner(ownerId)
        ...
        inner class PriceFilter {
            ...
            @Test
            fun `should filter by min price`() = runTest {
                val ownerId = createOwner()
                createOfferWithOwner(ownerId, unitPrice = 50.usd())
                val expensiveOffer = createOfferWithOwner(ownerId, unitPrice = 150.usd())
            ...
            @Test
            fun `should filter by max price`() = runTest {
                val ownerId = createOwner()
                val cheapOffer = createOfferWithOwner(ownerId, unitPrice = 50.usd())
            ...
            @Test
            fun `should filter by price range`() = runTest {
                val ownerId = createOwner()
                createOfferWithOwner(ownerId, unitPrice = 50.usd())
                val middleOffer = createOfferWithOwner(ownerId, unitPrice = 150.usd())
                ...
                assertThat(result.items).hasSize(1)
                assertThat(result.items.first().id).isEqualTo(middleOffer.id)
            }

            @Test
            fun `should include boundary values in price range`() = runTest {
                val ownerId = createOwner()
                val minOffer = createOfferWithOwner(ownerId, unitPrice = 100.usd())
                val maxOffer = createOfferWithOwner(ownerId, unitPrice = 200.usd())
            ...
            @Test
            fun `should return empty when no prices in range`() = runTest {
                val ownerId = createOwner()
                createOfferWithOwner(ownerId, unitPrice = 50.usd())
                createOfferWithOwner(ownerId, unitPrice = 300.usd())
        ...
        inner class CurrencyFilter {
            ...
            @Test
            fun `should filter by currency`() = runTest {
                val ownerId = createOwner()
                val usdOffer = createOfferWithOwner(ownerId, unitPrice = 50.usd())
            ...
            @Test
            fun `should return empty when currency not match`() = runTest {
                val ownerId = createOwner()
        ...
        inner class CombinedFilters {
            ...
            @Test
            fun `should combine owner and price filters`() = runTest {
                val ownerId1 = createOwner()
                val ownerId2 = createOwner()
                createOfferWithOwner(ownerId1, unitPrice = 50.usd())
                val expensiveOffer = createOfferWithOwner(ownerId1, unitPrice = 150.usd())
                ...
                assertThat(result.items).hasSize(1)
                assertThat(result.items.first().id).isEqualTo(expensiveOffer.id)
            }

            @Test
            fun `should combine product ids and price filters`() = runTest {
                val ownerId = createOwner()
                val productId = createProductWithOwner(ownerId)
                ...
                val expensiveOffer = persistence.persist(
                    offer(
                        storeId = createStoreWithOwner(ownerId),
                        productId = productId,
                        unitPrice = 150.usd()
            ...
            @Test
            fun `should combine store ids and currency filters`() = runTest {
                val ownerId = createOwner()
                val storeId = createStoreWithOwner(ownerId)

                val usdOffer = persistence.persist(
                    offer(
                        storeId = storeId,
            ...
            @Test
            fun `should combine all filters`() = runTest {
                val ownerId = createOwner()
                val productId1 = createProductWithOwner(ownerId)
                val productId2 = createProductWithOwner(ownerId)
                ...
                val matchingOffer = persistence.persist(
                    offer(
                        storeId = storeId1,
                ...
                assertThat(result.items).hasSize(1)
                assertThat(result.items.first().id).isEqualTo(matchingOffer.id)
            }
            ...
            @Test
            fun `should return empty when combined filters exclude all`() = runTest {
                val ownerId = createOwner()
                createOfferWithOwner(ownerId, unitPrice = 50.usd())
