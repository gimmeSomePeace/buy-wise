...
import me.gimmesomepeace.buywise.offer.OfferException
...
class InMemoryOfferRepository : OfferRepository {
    private val data = mutableMapOf<OfferId, Offer>()

    override suspend fun get(offerId: OfferId): Offer =
        data[offerId] ?: throw OfferException.NotFound(offerId)

    override suspend fun add(offer: Offer) {
        if (offer.id in data) throw OfferException.AlreadyExists(offer.id)
        data[offer.id] = offer
    }

    override suspend fun update(offer: Offer) {
        if (offer.id !in data) throw OfferException.NotFound(offer.id)
        data[offer.id] = offer
    }

    override suspend fun delete(offerId: OfferId) {
        if (offerId !in data) throw OfferException.NotFound(offerId)
        data.remove(offerId)
    }
