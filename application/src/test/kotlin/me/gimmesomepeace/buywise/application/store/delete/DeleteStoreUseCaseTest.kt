...
class DeleteStoreUseCaseTest {
    ...
    @Test
    fun `should delete store when it exists and belongs to user`() =
        runTest {
            val ownerId = userId()
            val storeId = storeId()
    ...
    fun `should throw NotFound when store belongs to another user`() =
        runTest {
            ...
            coEvery { repository.get(storeId) } returns
                store(id = storeId, ownerId = anotherUserId)
    ...
    fun `should throw NotFound when store not found`() =
        runTest {
            val storeId = storeId()
            coEvery { repository.get(storeId) } throws
                StoreException.NotFound(storeId)
