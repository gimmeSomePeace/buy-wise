...
class RegisterUserUseCaseTest {
    ...
    @Test
    fun `should fail when login is busy`() = runTest {
        val login = login()
        val idGenerator = ::userId
        ...
        every { passwordHasher.hash(any()) } returns PasswordHash("password")
        coEvery { repository.add(any()) } throws UserException.LoginBusy(login)

        val ex = assertFailsWith<UserException.LoginBusy> {
