...
class ChangeOfferPriceUseCaseTest {
    ...
    private val useCase =
        ChangeOfferPriceUseCase(
            repository = offerRepository,
            query = offerQuery,
        )

    @Test
    fun `should change price when offer exists and belongs to user`() =
        runTest {
            val ownerId = userId()
            val offerId = offerId()
            val storeId = storeId()
            val offer =
                offer(id = offerId, storeId = storeId, unitPrice = 100.usd())
            val newPrice = 200.usd()

            coEvery { offerRepository.get(offerId) } returns offer
            coEvery { offerQuery.existsByIdAndOwner(offerId, ownerId) } returns
                true
    ...
    fun `should throw NotFound when offer not found`() =
        runTest {
            val offerId = offerId()
            coEvery { offerRepository.get(offerId) } throws
                OfferException.NotFound(offerId)
            coEvery { offerQuery.existsByIdAndOwner(offerId, any()) } returns
                false
