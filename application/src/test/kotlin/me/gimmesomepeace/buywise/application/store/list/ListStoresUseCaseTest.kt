...
class ListStoresUseCaseTest {
    ...
    fun `should return stores for owner`() = runTest {
        val ownerId = userId()
        val page = Page(
            items = listOf(storeListItem(ownerId = ownerId)),
            cursor = null,
        )
    ...
    fun `should return empty page when no stores`() = runTest {
        val ownerId = userId()
        val emptyPage = Page<StoreListItem>(items = emptyList(), cursor = null)
...
//        val ownerId = userId()
//        val page = Page<StoreListItem>(items = emptyList(), cursor = null)
//
//        coEvery { query.list(any(), any()) } returns page
//
