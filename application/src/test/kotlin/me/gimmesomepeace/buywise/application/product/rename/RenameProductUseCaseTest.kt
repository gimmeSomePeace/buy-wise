...
class RenameProductUseCaseTest {
    ...
    @Test
    fun `should rename product when it exists and belongs to user`() =
        runTest {
            val ownerId = userId()
            val productId = productId()
            val product =
                product(id = productId, ownerId = ownerId, name = "Old Name")
    ...
    fun `should throw NotFound when product belongs to another user`() =
        runTest {
            ...
            coEvery { repository.get(productId) } returns
                product(id = productId, ownerId = anotherUserId)
    ...
    fun `should throw NotFound when product not found`() =
        runTest {
            val productId = productId()
            coEvery { repository.get(productId) } throws
