...
@RestControllerAdvice(
    assignableTypes = [StoreController::class],
)
internal class StoreExceptionHandler {
    @ExceptionHandler(
        StoreException.NotFound::class,
    )
    fun handleNotFound(exc: StoreException.NotFound) =
        ProblemDetail
            .forStatus(
                ...
                detail =
                    "Offer with id ${exc.storeId} not found"
            }

    @ExceptionHandler(
        StoreException.AlreadyExists::class,
    )
    fun handleAlreadyExists(exc: StoreException.AlreadyExists) =
        ProblemDetail
            .forStatus(
                ...
                detail =
                    "Offer with id ${exc.storeId} already exists"
            }
}
