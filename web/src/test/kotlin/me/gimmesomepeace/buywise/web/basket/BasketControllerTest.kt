...
class BasketControllerTest {
    ...
    inner class Get {
        @Test
        fun `should return basket`() = runTest {
            coEvery { basketQuery.find() } returns basketDetails(
                basketItemsDetails(),
                basketItemsDetails(),
            )

            val mvcResult = mockMvc.get("/basket") {
        ...
        @Test
        fun `should return empty basket when not found`() = runTest {
    ...
    inner class Add {
        ...
        fun `should add product to empty basket`() = runTest {
            ...
            val mvcResult = mockMvc.post("/basket/items") {
        ...
        fun `should fail when quantity is not positive`(quantity: Int) = runTest {
            mockMvc.post("/basket/items") {
                contentType = MediaType.APPLICATION_JSON
                content = objectMapper.writeValueAsString(
    ...
    inner class ChangeProductQuantity {
        @Test
        fun `should change product quantity`() = runTest {
            val productId = productId()

            val mvcResult = mockMvc.put("/basket/items/${productId.value}") {
                contentType = MediaType.APPLICATION_JSON
                content = objectMapper.writeValueAsString(
        ...
        @Test
        fun `should fail when quantity is negative`() = runTest {
            mockMvc.put("/basket/items/${productId().value}") {
                contentType = MediaType.APPLICATION_JSON
                content = objectMapper.writeValueAsString(
    ...
    inner class Remove {
        @Test
        fun `should remove product from basket`() = runTest {
            val productId = productId()
            val mvcResult = mockMvc.delete("/basket/items/${productId.value}")
                .andReturn()
        ...
        @Test
        fun `should fail when product is not in basket`() = runTest {
            val productId = productId()
